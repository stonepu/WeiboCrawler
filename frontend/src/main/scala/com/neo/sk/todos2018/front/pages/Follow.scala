...
class Follow(nickname: String)extends Index {
  val pageLimit = 15
  val infoList = Var(List.empty[UserInfo])

  def getFollowInfoByPage(nickname: String, page: Int) = {
    val url = Routes.Blog.getFollowInfo
  ...
  override def render: xml.Node = {
    getFollowInfoByPage(nickname, 1)
    <div class="container">
      <div class="row">
